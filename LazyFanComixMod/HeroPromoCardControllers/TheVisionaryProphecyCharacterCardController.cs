...
namespace LazyFanComix.TheVisionary
{
    public class TheVisionaryProphecyCharacterCardController : PromoDefaultCharacterCardController
    {
        ...
        public override IEnumerator UsePower(int index = 0)
        {
            ...
            new Function(this.HeroTurnTakerController, "Play a card from under this card.", SelectionType.DiscardCard,
                () => this.GameController.SelectAndPlayCard(this.HeroTurnTakerController, (Card c) => c.Location == this.Card.UnderLocation || c.Location == this.Card.BelowLocation, cardSource: this.GetCardSource()),
            ...
            SelectFunctionDecision sfd = new SelectFunctionDecision(this.GameController, this.HeroTurnTakerController, list, false, null, this.TurnTaker.Name + " cannot move the top card of their deck to under themselves or play a card from under themselves, so " + this.Card.Title + " has no effect.", null, this.GetCardSource());
