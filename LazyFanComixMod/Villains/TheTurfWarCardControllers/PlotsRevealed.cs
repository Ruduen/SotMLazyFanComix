...
namespace LazyFanComix.TheTurfWar
{
    public class PlotsRevealedCardController : CardController
    {
        ...
        public override IEnumerator Play()
        {
            ...
            coroutine = this.GameController.SelectAndDestroyCards(this.DecisionMaker, new LinqCardCriteria((Card c) => c.IsEnvironment && c.IsInPlayAndNotUnderCard), 1, false, 1, cardSource: this.GetCardSource());
            if (this.UseUnityCoroutines) { yield return this.GameController.StartCoroutine(coroutine); } else { this.GameController.ExhaustCoroutine(coroutine); }
        }
    }
}