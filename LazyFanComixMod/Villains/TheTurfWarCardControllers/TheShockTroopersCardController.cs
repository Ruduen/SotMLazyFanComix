...
{
    public class TheShockTroopersCardController : SharedMoveCardUnderSharedKeywordCardController
    {
        public TheShockTroopersCardController(Card card, TurnTakerController turnTakerController) : base(card, turnTakerController)
        {
        }
        ...
        private IEnumerator DestroyCardResponse(PhaseChangeAction pca)
        {
            ...
            coroutine = this.GameController.SelectAndDestroyCards(this.DecisionMaker, new LinqCardCriteria((Card c) => !this.IsVillain(c) && this.IsEquipment(c)), 2, storedResultsAction: dca, cardSource: this.GetCardSource());
        ...
        {
            IEnumerator coroutine = this.GameController.DealDamage(this.DecisionMaker, this.Card, (Card c) => !c.IsMinion, 2, DamageType.Lightning, cardSource: this.GetCardSource());
            if (this.UseUnityCoroutines) { yield return this.GameController.StartCoroutine(coroutine); } else { this.GameController.ExhaustCoroutine(coroutine); }
        }
    }
}
