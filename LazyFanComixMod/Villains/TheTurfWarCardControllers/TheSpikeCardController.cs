...
namespace LazyFanComix.TheTurfWar
{
    public class TheSpikeCardController : SharedMoveCardUnderSharedKeywordCardController
    {
        ...
        private IEnumerator StartDamageResponse(PhaseChangeAction pca)
        {
            ...
            Func<DealDamageAction, int> EnvironmentCards = (DealDamageAction dda) => this.FindCardsWhere((Card c) => c.IsInPlay && c.IsEnvironment).Count();

            // Increase trigger. 
            ITrigger tempIncrease = this.AddIncreaseDamageTrigger((DealDamageAction dda) => dda.CardSource.CardController == this, EnvironmentCards);

            IEnumerator coroutine = this.DealDamageToHighestHP(this.Card, 1, (Card c) => !c.IsThug, (Card c) => 3, DamageType.Melee, numberOfTargets: () => this.H);
