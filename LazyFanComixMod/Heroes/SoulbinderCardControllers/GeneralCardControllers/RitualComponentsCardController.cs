...
namespace LazyFanComix.Soulbinder
{
    public class RitualComponentsCardController : SoulbinderSharedYourTargetDamageCardController
    {
        ...
        public override IEnumerator UsePower(int index = 0)
        {
            ...
            // Soulbinder deals 1 target 2 damage.
            coroutine = this.GameController.SelectTargetsAndDealDamage(this.HeroTurnTakerController, new DamageSource(this.GameController, this.CharacterCard), powerNumerals[1], DamageType.Infernal, powerNumerals[0], false, powerNumerals[0], cardSource: this.GetCardSource());
        ...
        private IEnumerator RemoveTokenResponse(SelectCardDecision scd, int number)
        {
            if (scd.SelectedCard != null && scd.SelectedCard.FindTokenPool("RitualTokenPool").CurrentValue > 0)
            {
                ...
                coroutine = this.GameController.SendMessageAction(this.Card.AlternateTitleOrTitle + " removes " + number + " Ritual Token from " + scd.SelectedCard.AlternateTitleOrTitle + ".", Priority.Low, this.GetCardSource(), new Card[] { scd.SelectedCard }, true);
                if (this.UseUnityCoroutines) { yield return this.GameController.StartCoroutine(coroutine); } else { this.GameController.ExhaustCoroutine(coroutine); }
