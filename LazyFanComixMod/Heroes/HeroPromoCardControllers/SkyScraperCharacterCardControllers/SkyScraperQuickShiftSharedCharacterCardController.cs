  ...
  public abstract class SkyScraperQuickShiftSharedCharacterCardController : SkyScraperCharacterCardController
  {
    ...
    public override IEnumerator UsePower(int index = 0)
    {
      ...
      choices = new List<Function>() {
                new Function(this.DecisionMaker, "Play a " + cardKeyword() + " card", SelectionType.PlayCard, () => this.GameController.SelectAndPlayCardFromHand(this.DecisionMaker, false, cardCriteria: new LinqCardCriteria((Card c) => c != null && this.GameController.DoesCardContainKeyword(c, cardKeyword())), cardSource: this.GetCardSource()), this.DecisionMaker?.HeroTurnTaker?.Hand?.Cards != null && this.CanPlayCardsFromHand(this.DecisionMaker) && this.DecisionMaker.HeroTurnTaker.Hand.Cards.Any((Card c)=>this.GameController.DoesCardContainKeyword(c, cardKeyword())), this.TurnTaker.Name + " cannot draw any cards, so they must play a " + cardKeyword() + " card."),
