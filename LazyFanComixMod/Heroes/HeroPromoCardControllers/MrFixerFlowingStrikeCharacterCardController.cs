...
namespace LazyFanComix.MrFixer
{
    public class MrFixerFlowingStrikeCharacterCardController : PromoDefaultCharacterCardController
    {
        ...
        public override IEnumerator UsePower(int index = 0)
        {
            ...
            // Deal target damage.
            coroutine = this.GameController.SelectTargetsAndDealDamage(this.HeroTurnTakerController, new DamageSource(this.GameController, this.CharacterCard), numerals[1], DamageType.Melee, numerals[0], false, numerals[0], cardSource: this.GetCardSource());
            ...
            // Discard 3 cards.
            coroutine = this.GameController.DiscardTopCards(this.HeroTurnTakerController, this.TurnTaker.Deck, numerals[2], mcaResult, cardSource: this.GetCardSource());
            if (this.UseUnityCoroutines) { yield return this.GameController.StartCoroutine(coroutine); } else { this.GameController.ExhaustCoroutine(coroutine); }
            ...
            {
                coroutine = this.GameController.SelectAndPlayCard(this.HeroTurnTakerController, validPlays, true, cardSource: this.GetCardSource());
                if (this.UseUnityCoroutines) { yield return this.GameController.StartCoroutine(coroutine); } else { this.GameController.ExhaustCoroutine(coroutine); }
            }
            else
            {
