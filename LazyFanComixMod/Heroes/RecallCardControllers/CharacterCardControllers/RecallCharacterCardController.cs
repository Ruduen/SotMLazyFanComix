...
namespace LazyFanComix.Recall
{
  public class RecallCharacterCardController : PromoDefaultCharacterCardController
  {
    ...
    public override IEnumerator UsePower(int index = 0)
    {
      ...
      list.Add(new Function(this.HeroTurnTakerController, "Play a Paradox", SelectionType.PlayCard, () => this.GameController.SelectAndPlayCardsFromHand(this.HeroTurnTakerController, 1, false, cardCriteria: new LinqCardCriteria((Card c) => this.GameController.DoesCardContainKeyword(c, "paradox") && !this.GameController.IsLimitedAndInPlay(Card)), cardSource: this.GetCardSource()),
          this.TurnTaker.IsHero && this.HeroTurnTaker.Hand.Cards.Where((Card c) => this.GameController.DoesCardContainKeyword(c, "paradox")).Any(), this.TurnTaker.Name + " cannot draw any cards, so they must play a Paradox."));
