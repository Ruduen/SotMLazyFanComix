    ...
    public class StreamShotCardController : CassieRiverSharedCardController
    {
        ...
        public override IEnumerator Play()
        {
            ...
            coroutine = this.GameController.SelectCardAndDoAction(new SelectCardDecision(this.GameController, this.HeroTurnTakerController, SelectionType.MoveCard, this.GameController.FindCardsWhere((Card c) => c.Location == this.Riverbank().UnderLocation)),
                (SelectCardDecision d) => this.GameController.MoveCard(this.HeroTurnTakerController, d.SelectedCard, RiverDeck(), toBottom: true, evenIfIndestructible: true, storedResults: mcaResults, cardSource: this.GetCardSource()),
                false);
            ...
            coroutine = this.GameController.MoveCards(this.HeroTurnTakerController, RiverDeck().GetTopCards(1), Riverbank().UnderLocation, cardSource: this.GetCardSource());
            if (this.UseUnityCoroutines) { yield return this.GameController.StartCoroutine(coroutine); } else { this.GameController.ExhaustCoroutine(coroutine); }
        }
    }
}