    ...
    public class GooLauncherCardController : SharedHeroGunUnearnedCardController
    {
        ...
        protected override IEnumerator ClaimTrigger(PhaseChangeAction pca)
        {
            ...
            if (pca.ToPhase.TurnTaker.IsHero)
            {
                ...
                coroutine = this.GameController.SelectAndDiscardCards(httc, 3, false, 0, storedResults: dcas, cardSource: this.GetCardSource());
                if (this.UseUnityCoroutines) { yield return this.GameController.StartCoroutine(coroutine); } else { this.GameController.ExhaustCoroutine(coroutine); }

                if (dcas.Where((DiscardCardAction dca) => dca.WasCardDiscarded).Count() == 3)
                {
