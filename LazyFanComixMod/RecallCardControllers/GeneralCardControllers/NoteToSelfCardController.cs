...
namespace LazyFanComix.Recall
{
    public class NoteToSelfCardController : CardController
    {
        ...
        public override IEnumerator Play()
        {
            ...
            coroutine = this.GameController.SelectAndDestroyCards(this.DecisionMaker, new LinqCardCriteria((Card c) => c.Location == this.CharacterCard.UnderLocation), 1, cardSource: this.GetCardSource());
            if (this.UseUnityCoroutines) { yield return this.GameController.StartCoroutine(coroutine); } else { this.GameController.ExhaustCoroutine(coroutine); }
        }
    }
}