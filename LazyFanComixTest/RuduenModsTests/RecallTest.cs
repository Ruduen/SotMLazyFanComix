...
namespace LazyFanComixTest
{
  ...
  public class RecallTest : BaseTest
  {
    ...
    public void TestCardImmediateJump()
    {
      ...
      StartGame();
      DiscardAllCards(Recall);
      Card cardToPlay = PutInHand("Recalled");
      DecisionSelectCard = cardToPlay;

      QuickHandStorage(Recall);
      PlayCard("ImmediateJump");
      AssertIsInPlay(cardToPlay);
      AssertNotUsablePower(Recall, Recall.CharacterCard);
      QuickHandCheck(-1 + 1 + 1); // One play, one draw from power, one draw from card.
    ...
    public void TestCardGoingThroughTheMotionsSelfDamage()
    {
      ...
      QuickHPStorage(Recall, legacy);
      UsePower(legacy);
      DealDamage(Recall, legacy, 1, DamageType.Fire);
      DealDamage(Recall, Recall, 1, DamageType.Fire);
      QuickHPCheck(-1, -2);
    }
